...
with open('./data1/IMDB-MULTI.pkl', 'rb') as f:
...
""" for data, labels in test_loader: 
    # 数据加载到设备 data = data.to(device) labels = labels.to(device) 
    # # 前向传播 z = projection_head(backbone(data)) logits = classifier(z) predictions = torch.sigmoid(logits).round() 
    # # 对于二分类问题 
    # # 存储预测和标签 all_predictions.extend(predictions.cpu().numpy()) all_labels.extend(labels.cpu().numpy()) 
    # # 计算评估指标 accuracy = compute_accuracy(all_predictions, all_labels) print(f"Test Accuracy: {accuracy}")
...
roc_auc = roc_auc_score(all_labels, all_predictions_proba)  # 需要预测概率
